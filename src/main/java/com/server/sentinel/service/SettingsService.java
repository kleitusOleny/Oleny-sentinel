...
public class SettingsService {
    ...
    public SettingsService(@Value("${discord.webhook.url:}") String defaultDiscordUrl) {
        ...
        loadSettings(defaultDiscordUrl);
    }

    private synchronized void loadSettings(String defaultDiscordUrl) {
        ...
        if (file.exists()) {
            try {
                ...
                if (data.containsKey("discordWebhookUrl")) {
                    this.discordWebhookUrl = (String) data.get("discordWebhookUrl");
                }
                System.out.println("Da nap cau hinh tu file: " + settingsPath);
                return;
            } catch (IOException e) {
        ...
        // Neu file chua ton tai, dung default va luu vao file
        this.discordWebhookUrl = defaultDiscordUrl != null ? defaultDiscordUrl : "";
        saveSettingsToFile();
    }
    ...
    private synchronized void saveSettingsToFile() {
        try {
            ...
            data.put("discordWebhookUrl", this.discordWebhookUrl);
            
            objectMapper.writerWithDefaultPrettyPrinter().writeValue(file, data);
            System.out.println("Da luu cau hinh vao file: " + settingsPath);
        } catch (IOException e) {
    ...
    public synchronized void updateSettings(Map<String, Object> newSettings) {
        ...
        if (newSettings.containsKey("discordWebhookUrl")) {
            this.discordWebhookUrl = (String) newSettings.get("discordWebhookUrl");
        }
        saveSettingsToFile();
    }
}
